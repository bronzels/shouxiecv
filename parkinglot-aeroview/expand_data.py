"""
这个文件扩展数据集用， 目前train_data数据量还是太少，所以就打算把cnn_pred_data下面的数据，用模型预测，然后剪切到模型训练的相应目录下就行扩增

这个函数，以后就接受一张停车场的图片，视频的一帧，然后保存到test_images目录下面

然后从这个里面读取图片，然后通过数据预处理操作，把停车位划分开， 然后保存到cnn_pred_data里面，然后再从这里面读数据，预测，见铁道train_data/train中

这样，数据量打了，能提高模型的预测准确度， 因为发现目前模型预测的并不是很准
"""
import os
import json
