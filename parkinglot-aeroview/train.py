...
def train_model():
    ...
    train_loader, validate_loader, train_num, val_num = get_dataloader(train_data_path, val_data_path,
                                                                  data_transform_pretrain, batch_size=8)

    # 创建模型 注意这里没指定类的个数，默认是1000类
    #net = resnet34()
    net = resnet34(pretrained=True)
    #model_weight_path = 'saved_model_weight/resnet34_pretrain_ori_low_torch_versions.pth'
    ...
    model_train(net, train_loader, validate_loader, epochs, device, optimizer, loss_function, train_steps, val_num,
                save_path, best_acc)